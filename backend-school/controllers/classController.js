  ...
  try {
    ...
    if (!school_id) {
      return res.status(401).json({
        success: false,
        message: 'Unauthorized: School information not found.',
      });
    }
    ...
    // Check if class already exists in this school (case-insensitive)
    ...
    if (existingClass.length > 0) {
      return res.status(409).json({
        success: false,
        message: `Class '${trimmedClassName}' already exists in your school.`,
      });
    }
    ...
    res.status(201).json({
      success: true,
      message: '✅ Class added successfully (Ready for Divisions)',
...
// -------------------- Add Division (By Class Name) --------------------
exports.addDivision = async (req, res) => {
  try {
    const school_id = req.user?.school_id; // auto-filled
    ...
    if (!class_name || !division_name) {
      return res.status(400).json({
        success: false,
        message: 'Class Name and Division Name are required',
      });
    }
    ...
    // Get class_id using class_name and school_id
    ...
    if (classResult.length === 0) {
      return res.status(404).json({
        success: false,
        message: `Class '${class_name}' not found. Please create the class first.`,
      });
    }
    ...
    // Insert division
    const query = `
      INSERT INTO divisions (class_id, division_name, class_teacher, expected_students, created_at, updated_at)
      VALUES (:class_id, :division_name, :class_teacher, :expected_students, NOW(), NOW())
      RETURNING *;
    `;

    const [insertedDivision] = await sequelize.query(query, {
      replacements: { class_id, division_name, class_teacher, expected_students },
      type: QueryTypes.INSERT,
    });
